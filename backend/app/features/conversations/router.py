"""Conversation HTTP endpoints (workspace-scoped)."""
...
async def list_conversations(
    ...
    items, total = await service.list_conversations(workspace_user.id, pagination)
    return ApiResponse(
...
async def get_conversation(
    ...
    conversation, messages = await service.get_with_messages(
        workspace_user.id, conversation_id
    )
    return ApiResponse(
